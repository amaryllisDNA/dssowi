...
output:
  html_document:
    df_print: paged
---
#Der Mensch fördert und hemmt andere Arten in der städtischen Landschaft und prägt damit die Muster der biologischen Vielfalt.Institutioneller Rassismus kann der Verteilung städtischer Arten zugrunde liegen, indem er zeitlich und räumlich unverhältnismäßige Ressourcen schafft.durch die Schaffung unverhältnismäßiger Ressourcen in Raum und Zeit.ob der heutige Bestand, die Vielfalt und die Zusammensetzung von Straßenbäumen in New York City, USA widerspiegeln
...
df<-scale(data_cluster[,c("ESTAB","eth_afroam_nonhisp","eth_asian_nonhisp","eth_hisp","eth_other","pop_density","tree_density")])
...
df[is.na(df)] <- 0 # Ersetzt fehlende Werte durch 0



fviz_nbclust(df,kmeans,method="wss")+geom_vline(xintercept=6,linetype=2)+labs(title="Elbow methode")
```
